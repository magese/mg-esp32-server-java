...
import com.magese.ai.dao.ConfigMapper;
import com.magese.ai.dialogue.token.factory.TokenServiceFactory;
import com.magese.ai.entity.SysAgent;
import com.magese.ai.entity.SysConfig;
import com.magese.ai.entity.SysDevice;
import com.magese.ai.service.SysAgentService;
...
public class SysAgentServiceImpl implements SysAgentService {
                ...
                try {
                    ...
                    if (infoResponse.statusCode() == 200) {
                        ...
                        try {
                            ...
                            log.error("添加DIFY LLM配置失败: {}", e.getMessage());
                        }
    ...
    private List<SysAgent> getCozeAgents(SysAgent agent) {
        ...
        SysConfig config = configs.getFirst();
        ...
        try {
            ...
            if (response.statusCode() == 200) {
                ...
                if (rootNode.has("code") && rootNode.get("code").asInt() == 0) {
                    ...
                    for (JsonNode botNode : spaceBots) {
                        ...
                        if (existingConfigMap.containsKey(botId)) {
                            ...
                            try {
                                ...
                                log.error("更新智能体配置失败: {}", e.getMessage());
                            }
                        } else {
                            // 不存在则新增
                    ...
                    //             log.debug("删除智能体配置成功: {}", existingBotId);
                    //         } catch (Exception e) {
                    //             log.error("删除智能体配置失败: {}", e.getMessage());
                    //         }
                    //     }
                    // }
                } else {
