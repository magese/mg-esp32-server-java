package com.magese.ai.dialogue.tts.providers;
...
import com.magese.ai.dialogue.tts.TtsService;
...
public class AliyunTtsService implements TtsService {
        ...
        while (attempts < MAX_RETRY_ATTEMPTS) {
            try {
                ...
                Future<Boolean> downloadFuture = executor.submit(() -> {
                    try (InputStream in = new URI(audioUrl).toURL().openStream();
    ...
    private AudioParameters.Voice getVoiceByName(String voiceName) {
        return switch (voiceName) {
            case "Chelsie" -> AudioParameters.Voice.CHELSIE;
