package com.magese.ai.dialogue.llm.tool.mcp.device;

import com.magese.ai.communication.common.ChatSession;
import com.magese.ai.communication.domain.DeviceMcpMessage;
import com.magese.ai.communication.domain.mcp.device.DeviceMcpPayload;
import com.magese.ai.communication.domain.mcp.device.initialize.DeviceMcpClientInfo;
import com.magese.ai.communication.domain.mcp.device.initialize.DeviceMcpInitialize;
import com.magese.ai.communication.domain.mcp.device.initialize.DeviceMcpVision;
...
public class DeviceMcpService {
    ...
    @Value("${magese-ai.mcp:device:max.tools.count:32}")
    ...
    private void sendToolsList(ChatSession chatSession) {
        ...
        if (result != null) {
                ...
                for (Map<String, Object> tool : tools) {
                    ...
                    var toolCallback = FunctionToolCallback
                            .builder(funcName, (Map<String, Object> params, ToolContext toolContext) -> {
                                ...
                                if (response != null) {
                                    log.debug("SessionId: {},  MCP function call response: {}", chatSession.getSessionId(), response);
                                    //空指针
            ...
            if (nextCursor != null && !nextCursor.toString().isEmpty()) {
                ...
                log.debug("SessionId: {}, mcp tools loaded successfully", chatSession.getSessionId());
            }
        }
    }
