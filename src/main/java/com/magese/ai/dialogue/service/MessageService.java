...
public class MessageService {
    ...
    public void sendTtsMessage(ChatSession session, String text, String state) {
        if (session == null || !session.isOpen()) {
            log.warn("sendTtsMessage无法发送消息 - 会话已关闭或为null");
        }
        ...
        log.info("sendTtsMessage发送消息 - SessionId: {}, Message: {}", Objects.requireNonNull(session).getSessionId(), jsonMessage);
    ...
    public void sendSttMessage(ChatSession session, String text) {
        ...
        String jsonMessage = messageJson.toString();
        log.info("sendSttMessage发送消息 - SessionId: {}, Message: {}", Objects.requireNonNull(session).getSessionId(), jsonMessage);
        sendTextMessage(session, jsonMessage);
    }
    ...
    public void sendIotCommandMessage(ChatSession session, List<Map<String, Object>> commands) {
        ...
        ObjectNode messageJson = JsonUtil.OBJECT_MAPPER.createObjectNode();
        messageJson.put("session_id", Objects.requireNonNull(session).getSessionId());
    ...
    public void sendTextMessage(ChatSession chatSession, String message) {
        try {
            ...
            log.error("发送text消息时发生异常 - SessionId: {}, Error: {}", chatSession.getSessionId(), e.getMessage());
    ...
    public void sendBinaryMessage(ChatSession chatSession, byte[] opusFrame) {
        try {
            ...
            log.error("发送binary消息时发生异常 - SessionId: {}, Error: {}", chatSession.getSessionId(), e.getMessage());
