...
import com.magese.ai.dialogue.vad.impl.SileroVadModel;
...
public class VadService {
    ...
    private final SileroVadModel vadModel;
    ...
    private class VadState {
        ...
        public float getLastOriginalProb() {
            return originalProbs.isEmpty() ? 0.0f : originalProbs.getLast();
        }

        public float getLastProb() {
            return probs.isEmpty() ? 0.0f : probs.getLast();
        }
     ...
     * 检查会话是否已初始化
     */
    public boolean isSessionNotInitialized(String sessionId) {
        Object lock = getLock(sessionId);
        synchronized (lock) {
    ...
    public VadResult processAudio(String sessionId, byte[] opusData) {
        ...
        if (isSessionNotInitialized(sessionId)) {
            return null;
        }
        ...
        synchronized (lock) {
            try {
                ...
                /* if (state.getFrameCounter() % LOG_FRAME_INTERVAL == 0) {
                     ...
                     log.info("VAD状态 - SessionId: {}, 帧: {}, 概率: {}, 能量: {}, 阈值: {}",
                ...
                if (!state.isSpeaking() && isSpeech) {
                    ...
                    log.info("检测到语音开始 - SessionId: {}, 概率: {}, 能量: {}, 阈值: {}",
                    ...
                    if (silenceDuration > silenceTimeoutMs) {
                        ...
                        log.info("语音结束: {}, 静音: {}ms", sessionId, silenceDuration);
