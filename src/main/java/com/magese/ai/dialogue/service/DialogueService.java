...
import com.magese.ai.dialogue.service.VadService.VadStatus;
import com.magese.ai.dialogue.stt.SttService;
...
import com.magese.ai.event.ChatSessionCloseEvent;
...
import com.magese.ai.utils.EmojiUtils;
import com.magese.ai.utils.EmojiUtils.EmoSentence;
import lombok.Data;
...
public class DialogueService implements ApplicationListener<ChatSessionCloseEvent> {
    ...
    private final HuiBenService huiBenService;
    ...
    private void saveUserAudio(ChatSession session) {
        try {
            ...
            if (pcmFrames != null && !pcmFrames.isEmpty()) {
                ...
                log.debug("用户音频已保存: {}", path.toString());
            }
        } catch (Exception e) {
            log.error("保存用户音频失败: {}", e.getMessage(), e);
        }
    }
    ...
    private void handleSentence(
        ...
        SysDevice device = session.getSysDevice();
        SysRole role;
        if (device == null || (role = roleService.selectRoleById(device.getRoleId())) == null) {
            return;
        }
        ...
        // 使用虚拟线程异步生成音频文件
        Thread.startVirtualThread(() -> generateAudio(session, sessionId, sentence, emoSentence, isFirst, isLast, ttsConfig, voiceName));
    }
    ...
    private void handleTtsSuccess(TtsTask task, String audioPath) {
        ...
        // 记录日志
        log.info("句子音频生成完成 - 序号: {}, 对话ID: {}, 模型响应: {}秒, 语音生成: {}秒, 内容: \"{}\"",
    ...
    private void handleTtsFailure(TtsTask task, String reason) {
        ...
        if (task.retryCount <= MAX_RETRY_COUNT) {
            ...
            // 超过最大重试次数，标记为失败
            log.error("TTS任务失败 - 序号: {}, 已达最大重试次数, 原因: {}",
    ...
    private void saveAssistantResponse(ChatSession session) {
        ...
        try {
            ...
            if (audioPaths == null || audioPaths.isEmpty()) {
                log.warn("对话 {} 没有可用的音频路径", assistantTimeMillis);
                return;
            }
            ...
            if (!audioFilesToMerge.isEmpty()) {
                ...
                // 保存合并后的音频路径
                log.info("对话 {} 的音频已合并: {}", assistantTimeMillis, path);
            }
        } catch (Exception e) {
            log.error("保存助手响应失败 - 对话ID: {}, 错误: {}", assistantTimeMillis, e.getMessage(), e);
        }
    }
    ...
    private void processQueue(ChatSession session, String sessionId) {
        ...
        try {
            ...
            if (nextSentence != null) {
                ...
                //log.info("是否最后一个句子{}, 对话ID: {}",nextSentence.isLast, nextSentence.assistantTimeMillis);
