...
public class DeviceController extends BaseController {
    ...
    public ResponseEntity<byte[]> ota(@RequestHeader("Device-Id") String deviceIdAuth, @RequestBody String requestBody,
        ...
        try {
            ...
            if (deviceIdAuth == null || cmsUtils.isMacAddressInvalid(deviceIdAuth)) {
            ...
            if (ObjectUtils.isEmpty(queryDevice)) {
                ...
                // 设备已绑定，更新设备状态和信息
                SysDevice boundDevice = queryDevice.getFirst();
                // 保留原设备名称，更新其他信息
    ...
    public ResponseEntity<String> otaActivate(@Parameter(name = "Device-Id", description = "设备唯一标识", required = true, in = ParameterIn.HEADER)
        ...
        try {
            if (cmsUtils.isMacAddressInvalid(deviceId)) {
            ...
            log.info("OTA激活结果查询成功, deviceId: {} 激活时间: {}", deviceId, sysDevice.getCreateTime());
        } catch (Exception e) {
            log.error("OTA激活失败", e);
            return ResponseEntity.status(202).build();
        }
